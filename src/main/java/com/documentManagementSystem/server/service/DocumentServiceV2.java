    ...
    public void deleteAllDocumentsByFolder(Long folderId, String username) {
        ...
        Folder folder = folderRepository.findById(folderId)
            .orElseThrow(() -> {
                ...
                return new RuntimeException("Folder not found with ID: " + folderId);
            });
        ...
        try {
            ...
            log.error("Error deleting documents from folder ID: {} for user: {}, error: {}", 
                    folderId, username, e.getMessage(), e);
            throw new RuntimeException("Failed to delete documents in folder ID " + folderId + ": " + e.getMessage());
        }
    }
    @Transactional
